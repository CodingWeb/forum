...
// récupérer les dernier topic

function get_topics_dernier()
{
    require 'connect.php';
    $requete = $bdd->prepare('SELECT * FROM topics ORDER BY id DESC LIMIT 0, 3');
    $requete->execute();
    $data = $requete->fetchAll();
    return $data;
    $requete->closeCursor();
...
function get_titre($titre)
{
    require 'connect.php';
    $requete = $bdd->prepare('SELECT * FROM topics WHERE titre=:titre');
...
// Envoyer un message a celui qui répond au sujet

function get_auteur_mail($membre_id)
{
    require 'connect.php';
    $requete = $bdd->prepare('SELECT email FROM membres WHERE id=:membre_id');
    $requete->bindValue(':membre_id', $membre_id, PDO::PARAM_STR);
    $requete->execute();
    $data = $requete->fetchObject();
    $emails = $data->email;
    return $emails;
...
// ajouter un commentaire a un sujet

function add_commentaire($message, $membre_id, $topic_id)
{
    require 'connect.php';
    $requete = $bdd->prepare('INSERT INTO forum.commentaires (message, topic_id, membre_id) VALUES (:message, :topic_id, :membre_id)');
    $requete->bindValue(':message', $message, PDO::PARAM_STR);
    $requete->bindValue(':topic_id', $topic_id, PDO::PARAM_STR);
    $requete->bindValue(':membre_id', $membre_id, PDO::PARAM_STR);
    $requete->execute();
    $requete->closeCursor();
}

function get_commentaire($topic_id)
{
    require 'connect.php';
    $requete = $bdd->prepare('SELECT * FROM forum.commentaires WHERE topic_id = :topic_id');
    $requete->bindValue(':topic_id', $topic_id, PDO::PARAM_STR);
    $requete->execute();
    $data = $requete->fetchAll();
    return $data;
...
{
    require 'connect.php';
    $requete = $bdd->prepare('SELECT pseudo FROM forum.membres WHERE id = :article_id');
    $requete->bindValue(':article_id', $membre_id, PDO::PARAM_STR);
    $requete->execute();
    $data = $requete->fetchObject();
    return $data;
    $requete->closeCursor();
